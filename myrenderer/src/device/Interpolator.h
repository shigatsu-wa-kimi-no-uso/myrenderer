...
class Interpolator {
public:
	Vec3 vertex_view_coords[3]; //静态,只有这一个实例,并发情况下需改成多Interpolator非静态变量,或静态变量列表

	Vec3 computeBarycentric2D(const Point2& p, const Vec4(&v)[3]) {
		double x = p.x(), y = p.y();
	...
	template<class T>
	T screenspace_interpolate(const Vec3& bar, const T(&v)[3]) {
		return bar.x() * v[0] + bar.y() * v[1] + bar.z() * v[2];
	}
	
	template<class T>
	T viewspace_interpolate(const Vec3& bar, const T(&v)[3]) {
